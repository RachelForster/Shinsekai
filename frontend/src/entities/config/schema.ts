...
export const systemConfigFormSchema: Array<FormGroupSchema<SystemConfig>> = [
  ...
  },
  {
    id: "network-proxy",
    title: "系统代理",
    description: "启用后保存，会写入当前进程的标准代理环境变量；新发起的网络请求会立即使用这些设置。",
    fields: [
      {
        description: "关闭后不会写入下面的代理地址，保存时会恢复进程启动时的代理环境。",
        label: "启用代理配置",
        name: "network_proxy_enabled",
        type: "checkbox",
      },
      {
        disabledReason: "开启代理配置后可编辑。",
        disabledWhen: (draft) => !draft.network_proxy_enabled,
        description: "写入 HTTP_PROXY / http_proxy。示例：http://127.0.0.1:7890",
        label: "HTTP 代理",
        name: "http_proxy_url",
      ...
      },
      {
        disabledReason: "开启代理配置后可编辑。",
        disabledWhen: (draft) => !draft.network_proxy_enabled,
        description: "写入 HTTPS_PROXY / https_proxy。大多数本地代理也使用 http://127.0.0.1:7890。",
        label: "HTTPS 代理",
      ...
      },
      {
        disabledReason: "开启代理配置后可编辑。",
        disabledWhen: (draft) => !draft.network_proxy_enabled,
        description: "写入 ALL_PROXY / all_proxy。支持 socks5:// 或 socks5h://。",
        label: "SOCKS5 代理",
