...
def test_invalid_final_answer_is_repaired_without_tools() -> None:
    adapter = MockLLMAdapter(
        responses=['{"dialog":[{"character_name":"Alice","sprite":"0","speech":"Hi"}]}']
    )
    manager = LLMManager(adapter=adapter, user_template="Return dialogue JSON.")
    source_messages = [
        {"role": "system", "content": "Return dialogue JSON."},
        {"role": "user", "content": "hello"},
    ]

    repaired = manager._repair_invalid_dialog_output(
        "Hello, this is plain text.", source_messages, {"temperature": 0.4}
    )

    assert '"dialog"' in repaired
    assert len(adapter.call_history) == 1
    call = adapter.call_history[0]
    assert call["stream"] is False
    assert call["kwargs"]["tools"] is None
    assert call["messages"][:2] == source_messages
    assert call["messages"][-2]["role"] == "assistant"
    assert call["messages"][-2]["content"] == "Hello, this is plain text."
...
def test_invalid_final_answer_retries_with_escalated_instruction() -> None:
    adapter = MockLLMAdapter(
        responses=[
            "still not json",
            '{"dialog":[{"character_name":"Alice","sprite":"0","speech":"Fixed"}]}',
        ]
    )
    manager = LLMManager(adapter=adapter, user_template="Return dialogue JSON.")
    source_messages = [
    ...
    repaired = manager._repair_invalid_dialog_output(
        "plain text answer", source_messages, {"temperature": 0.4}
    )

    assert '"dialog"' in repaired and '"Fixed"' in repaired
    assert len(adapter.call_history) == 2
    # MockLLMAdapter records the mutated message list by reference, so inspect the
    # final accumulated state: the gentle reformat must precede the escalated retry.
    final_messages = adapter.call_history[-1]["messages"]
    contents = [m.get("content", "") for m in final_messages]
    repair_idx = next(i for i, c in enumerate(contents) if "Reformat" in c)
    retry_idx = next(i for i, c in enumerate(contents) if "MUST" in c)
    assert repair_idx < retry_idx
    # The still-invalid first reply is shown back to the model before retrying.
    assert {"role": "assistant", "content": "still not json"} in final_messages
    # Every repair attempt is tool-free so JSON mode can be enabled.
...
def test_invalid_final_answer_gives_up_after_max_attempts() -> None:
    ...
    result = manager._repair_invalid_dialog_output(
        "plain text answer",
        [{"role": "system", "content": "S"}, {"role": "user", "content": "hi"}],
        {},
    )

    assert result == "plain text answer"  # both attempts failed → original returned
    assert len(adapter.call_history) == 2
