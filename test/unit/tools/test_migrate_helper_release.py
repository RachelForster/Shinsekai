...
@pytest.mark.parametrize(
    ("assets", "system", "machine"),
    [
        (["Shinsekai_0.1.0_amd64.AppImage"], "Windows", "AMD64"),
        (["latest.json"], "Windows", "AMD64"),
        (["Shinsekai-migration-helper.exe"], "Windows", "AMD64"),
        (
            [
                "Shinsekai_0.1.0_x64-setup.exe",
                "Shinsekai_0.1.0_x64_en-US.msi",
            ],
            "Windows",
            "arm64",
        ),
        (["Shinsekai_0.1.0_x86_64-setup.exe"], "Windows", "x86"),
        (["Shinsekai_0.1.0_x64.dmg"], "Darwin", "arm64"),
        (["Shinsekai_0.1.0_amd64.AppImage"], "Linux", "arm64"),
        (["Shinsekai_0.1.0_amd64.AppImage"], "Darwin", "x64"),
        (["Shinsekai_0.1.0_x64-setup.exe"], "Linux", "x64"),
        (["Shinsekai_0.1.0_x64-setup.exe"], "Plan9", "x64"),
    ],
)
def test_platform_does_not_fall_back_to_unrelated_asset(
    assets: list[str],
    system: str,
    machine: str,
) -> None:
    assert (
        select_release_asset(
            [_asset(name) for name in assets],
            system=system,
            machine=machine,
        )
        is None
...
def test_unique_download_path_adds_suffix_when_file_exists(tmp_path) -> None:
    ...
    assert unique_download_path(tmp_path, "Shinsekai.msi") == (
        tmp_path / "Shinsekai-1.msi"
    )
