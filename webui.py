...
6. 旁白描写是场景动作描写
7. 选项必须是dialog最后一个元素，不能出现在其他地方，你必须在dialog最后一个元素中添加选项，选项元素的character_name必须是选项，内容在speech内，选项如果多于两个请用"/"分隔，xx选项必须是用户可以选择的对话、行为等，选项中不能出现任何多余的描述和内容，必须是纯文本。选项里有一个是纯粹插科打诨，无厘头的。\n
8. 数值可以用富文本，可以添加颜色等表示，例如 <span style='color:xxxx;'>HP：100</span>，选项元素的character_name必须是数值，内容在speech内，如果有多个数值，请用换行符分隔开，在这里，数值代表角色对用户的好感度。
"""
    template += "\n请开始对话:\n"
    return template, ""
