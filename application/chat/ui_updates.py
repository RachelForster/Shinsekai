...
class StreamingUIUpdateManager(HeadlessUIUpdateManager):
    ...
    复刻 ``connect_to_desktop_window`` 的下行契约：生产者（application handlers 的
...
def connect_to_stream_sink(ui: Any, sink: "ChatEventSink") -> None:
    """Mirror a legacy desktop presenter into the framework-neutral stream."""

    mirror = StreamingUIUpdateManager(
        sink,
        chat_history=ui.chat_history,
        bg_group=ui.bg_group,
    )
    ...
    original_post_tool_confirmation = getattr(
        ui,
        "post_tool_confirmation",
        None,
    )
    original_clear_tool_confirmation = getattr(
        ui,
        "clear_tool_confirmation",
        None,
    )
    ...
    original_set_user_display_name = getattr(
        ui,
    ...
    def update_sprite_from_path(
        ...
        *,
        ...
        ok = original_update_sprite_from_path(
            image_path,
            character_name=character_name,
            scale=scale,
        )
        if ok:
            mirror.update_sprite_from_path(
                image_path,
                character_name=character_name,
                scale=scale,
            )
