"""Plugin frontend contribution and configuration use cases."""
...
def _plugin_frontend_child(root: Path, value: str) -> Path:
    target = root
    for component in value.split("/"):
        target = managed_child_path(
            target,
            component,
            field="plugin frontend asset component",
        )
    return target
    ...
    *,
    ...
    if not raw_asset:
        ...
        if any(part in {"", ".", ".."} for part in parts):
            raise ValueError("plugin frontend asset must be an exact relative URL path")
        target = _plugin_frontend_child(root, cleaned_asset)
    if target.is_dir():
        relative = target.relative_to(root) / "index.html"
        target = _plugin_frontend_child(root, relative.as_posix())
