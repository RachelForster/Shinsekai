...
def _apply_registry_author_fallback(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    try:
        ...
        from plugin_system.registry.download import normalize_manifest_entry, normalize_repo_slug
...
def _plugin_rows(plugin_load: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    try:
        ...
        from plugin_system.registry.download import load_plugin_install_metadata
    except Exception:
        return []
...
def _plugin_registry_rows() -> list[dict[str, Any]]:
    from plugin_system.registry.catalog import fetch_registry_plugins
    from plugin_system.registry.download import load_downloaded_repos, normalize_manifest_entry, normalize_repo_slug
...
def _set_plugin_enabled(plugin_id: str, enabled: bool) -> dict[str, Any]:
    from plugin_system.host import set_plugin_manifest_enabled
...
def _uninstall_plugin(plugin_id: str) -> dict[str, Any]:
    from plugin_system.host import infer_plugin_package_directory, remove_plugin_manifest_entry
    from plugin_system.registry.download import unmark_repo_for_manifest_entry
