"""Qt dialog for guiding users during the React frontend migration."""
...
from tools.migrate_helper.release import (
    RELEASES_URL,
...
class MigrationRoleDialog(QDialog):
    """Choose the migration path for React frontend users."""
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        subtitle = QLabel(
            "产品界面已从 Qt 迁移到 React Frontend。请选择身份，我会展示对应的下一步。",
            hero,
        )
    ...
    def _developer_panel(self) -> QWidget:
        ...
        body = QLabel(
            "适合从源码参与 React Frontend 迁移、调试 bridge、开发页面和提交 PR。",
            panel,
        )
        ...
        steps = QLabel(
            ...
            "2. 在 frontend 目录安装依赖。\n"
            "3. 按 README 启动 React 界面和 Python bridge。",
            panel,
        )
        ...
        readme_btn.clicked.connect(
            lambda: self._open_local_file(_ROOT / "frontend" / "README.md")
        )
        root_readme_btn = QPushButton("查看项目 README", panel)
        root_readme_btn.clicked.connect(
            lambda: self._open_local_file(_ROOT / "README.md")
        )
        frontend_dir_btn = QPushButton("打开 frontend 目录", panel)
        frontend_dir_btn.clicked.connect(
            lambda: self._open_local_file(_ROOT / "frontend")
        )
    ...
    def _user_panel(self) -> QWidget:
        ...
        body = QLabel(
            "适合只想使用新版 React Frontend 的用户。点击后会自动匹配当前平台的最新"
            "发行包，下载到本机 Downloads/Shinsekai，并在下载完成后直接打开安装包。",
            panel,
        )
        body.setObjectName("migrationBody")
        body.setWordWrap(True)
        ...
        install_notice = QLabel(
            "安装提示：稍后安装时可以直接选择当前 Shinsekai 文件夹，安装器不会覆盖 "
            "data 和 plugins 文件夹。若你修改过其他源码或程序文件，请先备份或换目录"
            "安装，因为这些文件可能会被覆盖。",
            panel,
        )
        install_notice.setObjectName("migrationNotice")
        install_notice.setWordWrap(True)
        install_notice.setTextInteractionFlags(
            Qt.TextInteractionFlag.TextSelectableByMouse
        )
        layout.addWidget(install_notice)

        self._download_status = QLabel(
            "建议运行 exe / msi / dmg / AppImage 等发行包，而不是源码压缩包。",
            panel,
        )
