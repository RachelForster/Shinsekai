"""Dialogue-output validation and tool-free repair policy."""
...
_DIALOG_FORMAT_RETRY_PROMPT = (
    ...
    "`speech`. Output that JSON and nothing else: no prose, no markdown fences, no "
    "tool calls."
)
...
class ChatAdapter(Protocol):
    def chat(
        self, messages: list[dict], stream: bool = False, **kwargs: Any
    ) -> Any: ...
    ...
    *,
    ...
    for attempt in range(attempts):
        ...
        prompt = (
            _DIALOG_FORMAT_REPAIR_PROMPT
            if attempt == 0
            else _DIALOG_FORMAT_RETRY_PROMPT
        )
...
def repair_if_needed(
    *,
    ...
    """Apply the dialogue contract and report whether valid replacement text exists."""
    if not required or has_valid_dialog_output(content):
        return RepairOutcome(content=content)

    repaired = repair_dialog_output(
        adapter,
        content,
        messages,
        generation_kwargs,
    ...
    return RepairOutcome(
        content=repaired,
        repaired=repaired != content and has_valid_dialog_output(repaired),
    )
