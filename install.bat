...
if not exist "%PROJECT_ROOT%requirements.txt" (
    echo Error: installer could not identify the Shinsekai project root:
    echo "%PROJECT_ROOT%"
    pause
    exit /b 1
)
...
if defined CONDA_CMD goto :install_with_conda
call :resolve_command conda
if not errorlevel 1 (
    set "CONDA_CMD=%SHINSEKAI_RESOLVED_COMMAND%"
    goto :install_with_conda
)
...
:install_with_python
"%PYTHON_EXE%" -m pip install -r "%PROJECT_ROOT%requirements.txt" -i https://mirrors.aliyun.com/pypi/simple --extra-index-url https://pypi.tuna.tsinghua.edu.cn/simple --extra-index-url https://pypi.org/simple
...
"%CONDA_CMD%" run --cwd "%PROJECT_ROOT%" -n "%CONDA_ENV_NAME%" "%SHINSEKAI_CONDA_PYTHON%" -m pip install -r "%PROJECT_ROOT%requirements.txt" -i https://mirrors.aliyun.com/pypi/simple --extra-index-url https://pypi.tuna.tsinghua.edu.cn/simple --extra-index-url https://pypi.org/simple
set "SHINSEKAI_EXIT_CODE=%ERRORLEVEL%"

REM Check if installation succeeded
:install_finished
if not "%SHINSEKAI_EXIT_CODE%"=="0" (
    echo.
