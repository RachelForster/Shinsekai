...
- `config/models/` 保存 schema、feature flag、provider 默认值和纯规范化规则；
- `config/persistence/` 保存 YAML、MCP 配置及角色/背景受管资源的具体文件实现；
- `config/environment/` 保存 proxy、mirror、TTS 运行路径和进程环境适配；
- 更新所有生产、测试和 Tauri 资源引用，不保留根目录兼容转发模块；
- 增加子目录依赖守卫，禁止 models 反向依赖 persistence/environment，禁止
  environment 依赖 persistence。
...
| `llm/tools/mcp_config_file.py` | `config/persistence/mcp_config.py` | O4/O5 | 已完成；旧路径已删除 |
