...
程序内的“聊天主题”页面支持直接上传和导出 ZIP。进入“自定义主题”后：

- 基础模式用于常用颜色、排版和布局；高级模式覆盖完整 token、状态样式和日志页面；
- 内置主题保持只读，首次保存会生成用户主题副本；
- 用户主题可在资源字段旁上传图片、SVG、字体或音效，返回的相对路径会自动写入字段；
- 高级模式底部的资源工作台可查看和删除未被清单引用的资源；
- 主题卡片的“导出主题”会生成可再次上传安装的 ZIP。

资源被 `theme.json` 引用时不能直接删除。先清除对应字段并保存主题，再从资源工作台删除，避免产生失效引用。

聊天窗口中的外观按“全局默认 → 当前主题 → 当前会话”依次覆盖。会话面板中的值保持默认时会继承主题；显式修改后只覆盖当前使用体验。点击“将当前外观保存为主题”会复制当前主题及其资源，并把颜色、字体、打字速度、对话框透明度/缩放和窗口缩放固化到新的用户主题，然后清除这些会话覆盖，让新主题重新成为外观来源。立绘位置、自动隐藏和发送行为等内容相关或行为设置不会写入主题。

对应的可序列化字段包括 `tokens.global.windowScale`、`tokens.dialog.opacity`、`tokens.dialog.scale` 和 `tokens.dialog.fontFamily`。前三项范围分别为 `0.8–1.2`、`0.35–1`、`0.8–1.2`；未填写时继续使用宿主默认值或会话覆盖。
