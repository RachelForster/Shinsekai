...
日志工具条、侧栏和查看器的边框颜色与圆角固定使用宿主设置页的 `--color-border` 和 `--radius-panel`；展开条目的详情框使用 `--color-border` 和 `--radius-control`，并禁用主题阴影。它们不会读取 `logs.*.frame*`。旧主题中的日志 frame 字段仍可通过 `schema: 1` 校验，但会被前端忽略。
